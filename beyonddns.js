...
browser.proxy.onProxyError.addListener(function (error) {
  console.error('BEYOND-DNS: PAC error: ' + error.message);
});
...
browser.webRequest.onCompleted.addListener(function (details) {
  ...
  if (url) {
    console.log('BEYOND-DNS: #' + details.requestId + ' (' + url.domain + '): completed, ' + details.statusCode); //-
...
browser.webRequest.onBeforeRequest.addListener(function (details) {
  ...
  if (url) {
    ...
    if (ips) {
      console.log('BEYOND-DNS: #' + details.requestId + ' (' + url.domain + '): already resolved to ' + ips + '; cache size = ' + cache.length); //-
      ...
      console.log('BEYOND-DNS: #' + details.requestId + ' (' + url.domain + '): resolving at ' + (new Date).toTimeString() + ', full URL: ' + url.url); //-
      ...
      return new Promise(function (resolve, reject) {
        resolveViaAPI(url.domain, true, function (ips) {
          if (!ips) {
            ...
            console.log('BEYOND-DNS: #' + details.requestId + ': originUrl: ' + details.originUrl); //-
...
browser.webRequest.onErrorOccurred.addListener(function (details) {
  ...
  var url = parseURL(details.url);
  console.log('BEYOND-DNS: #' + req + ' (' + url.domain + '): ' + details.error); //-
  ...
  case 'NS_ERROR_NET_ON_CONNECTING_TO':
    if (cache.has(url.domain)) {
      ...
      if (cache.isExpired(url.domain, downCacheTTL)) {
        console.log('BEYOND-DNS: ' + url.domain + ': down, removing to refetch'); //-
...
browser.alarms.onAlarm.addListener(function () {
  ...
  console.log('BEYOND-DNS: deleted ' + count + ' expired entries; cache size = ' + cache.length); //-
});
...
browser.tabs.onUpdated.addListener(function (id, changeInfo) {
  ...
  if (url) {
    ...
    console.info('BEYOND-DNS: tab #' + id + ' updated to ' + (supported ? '' : 'un') + 'supported TLD, domain: ' + url.domain); //-
